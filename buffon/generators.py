...
desriptios = ("Генератор случайных чисел randint из "
              "стандартной библиотели Python random. Использует линейный "
              "конгруэнтный метод генерации случайных чисел.",
              "Генератор случайных чисел urandom из библиотеки "
              "Python OS. Генерирует зависимые от операционной системы "
              "случайные байты, которые можно назвать криптографически надежными.",
              "Генератор фон Неймана или метод срединных квадратов."
              " Начальное случайное число (зерно) возводится в квадрат, из результата берется середина и шаг повторяется "
